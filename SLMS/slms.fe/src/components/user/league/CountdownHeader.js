...
export default function CountdownHeader({ onBtnCircleClick }) {

    const { organizerId } = useParams();
    const { idLeague } = useParams();
    const location = useLocation();
    const [activeItem, setActiveItem] = useState("Đăng ký thi đấu"); // Mặc định item "Chung" được chọn
    ...
    const menuItems = useMemo(() => [
        {
            ...
            linknav: `/league/${organizerId}/dashboard/${idLeague}`
        },
        {
            id: 2,
            title: "Danh sách đăng ký",
            linknav: `/league${organizerId}/list-register//${idLeague}/calendarleague`
        },

    ], [organizerId, idLeague]);
                    ...
                    <Col style={{ display: "flex", justifyContent: "center", padding: "0" }}>
                        <div>
                            {leagueDetail.avatarTournament === null && (
                                <img src={images.champion} alt="Hình ảnh giải đấu" className={cx("user-image-preview")} />
                            )}
                            {leagueDetail.avatarTournament !== null && (
